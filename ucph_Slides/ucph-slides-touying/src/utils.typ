...
#let cover-with-rect(..cover-args, fill: auto, inline: true, body) = {
  ...
  let to-display = layout(layout-size => {
    context {
      ...
      stack(spacing: -wrapped-body-size.height, body, rect(fill: fill, ..named, ..cover-args.pos()))
    }
  })
  if inline {
    box(to-display)
  ...
  if stretch {
    context {
      ...
      for (subslides, content) in subslides-contents {
        only(self: self, subslides, box(width: max-width, height: max-height, align(position, content)))
      }
    }
  } else {
    for (subslides, content) in subslides-contents {
